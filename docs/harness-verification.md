...
tether has two genuinely different kinds of harness-facing surface, and they
...
agent or harness invokes it, because from tether's side there is no way to
...
(the test suite, `tether demo`) verifies it for every harness.

## Hooks (`tether hooks install`, Claude Code only)

Hooks are different: they mean writing into a specific harness's own
configuration format and trusting that harness to invoke tether at the
...
| Codex | (none implemented) | Unverified. No hook mechanism has been investigated or built for Codex. | — |
| OpenCode | (none implemented) | Unverified. No hook mechanism has been investigated or built for OpenCode. | — |
| Aider, Gemini CLI, Cline, and everything else | (none implemented) | Not attempted. These harnesses use the CLI-only path (`tether wait`, polling or blocking), which needs no hook and no per-harness verification — see above. | — |
