...
never touching your real ~/.tether), registers two agents named frontend and
...
func runDemo(cmd *cobra.Command) error {
	...
	dir, err := os.MkdirTemp("", "tether-demo")
	if err != nil {
	...
	if _, err := fmt.Fprintf(out,
		"tether demo -- spinning up an isolated daemon at %s (never your real ~/.tether)\n\n", sock); err != nil {
		return err
	}
...
// ~/.tether or collide with an already-running daemon.
func demoEnv(dir string) []string {
	return []string{
		"PATH=" + os.Getenv("PATH"),
		"HOME=" + dir,
		"TETHER_SOCK=" + filepath.Join(dir, "sock"),
		"TETHER_DB=" + filepath.Join(dir, "tether.db"),
	}
}

// demoStep is one line of the scripted handoff: which agent runs it, and the
// tether CLI arguments to run.
...
func runDemoStep(out io.Writer, exe string, env []string, step demoStep) error {
	...
	c.Env = append(append([]string{}, env...), "TETHER_SESSION_ID=demo-"+step.agent)
	...
	if runErr != nil {
		return failf(exitGeneral, "tether %s failed: %v", displayArgs(step.args), runErr)
	}
	return nil
}
