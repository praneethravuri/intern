...
// runRoot is root's RunE for a bare `tether` invocation: a live inbox
...
func runRoot(cmd *cobra.Command) error {
	...
	if len(ls.Agents) == 0 {
		_, err := fmt.Fprintf(out,
			"no agent registered in %s yet — register with `tether register --as <name>`\n", workspace)
		return err
	}
	...
	if inbox.Pending == 0 {
		...
		return next(out, "tether wait --timeout 5m")
	}
	if _, err := fmt.Fprintf(out, "%d unread for %s\n", inbox.Pending, addr); err != nil {
		return err
	}
	return next(out, "tether inbox")
}
