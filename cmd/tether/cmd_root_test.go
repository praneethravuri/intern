...
// MethodInbox (a --peek call, since bare tether never drains) with pending.
...
func TestBareTetherShowsUnreadCount(t *testing.T) {
	...
	requireContains(t, r.stdout, "Next: tether inbox", "stdout")

	inboxReq := d.requests()[2]
	params := decodeParams[protocol.InboxParams](t, inboxReq)
	if !params.Peek {
		t.Fatal("bare tether drained the inbox instead of peeking -- it must never clear mail")
	}
}

func TestBareTetherShowsZeroUnread(t *testing.T) {
	setIdentity(t, "frontend", "storefront")
	...
	requireContains(t, r.stdout, "Next: tether wait --timeout 5m", "stdout")
}

// TestBareTetherDoesNotRegisterInAnEmptyWorkspace is the deliberate decision
// behind principle 9 here: a workspace nobody has ever registered in stays
// that way after a bare `tether` glance -- it is a read, not a side effect.
func TestBareTetherDoesNotRegisterInAnEmptyWorkspace(t *testing.T) {
	...
	requireContains(t, r.stdout, "tether register --as <name>", "stdout")
	d.only(t, protocol.MethodLs)
}

func TestBareTetherWithoutADaemonExitsThree(t *testing.T) {
	setIdentity(t, "frontend", "storefront")
	noDaemon(t)
