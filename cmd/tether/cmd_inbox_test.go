...
func TestInboxEmpty(t *testing.T) {
	...
	requireContains(t, r.stdout, "Next: tether wait", "stdout")
...
func TestInboxToleratesOddTimestamps(t *testing.T) {
	setIdentity(t, "frontend", "storefront")
	newFakeDaemon(t, okHandler(protocol.InboxResult{Messages: []protocol.MessageView{{
		ID: "01K", From: "codex@storefront", Kind: kindNote, Body: "hi", CreatedAt: "yesterday",
	}}}))
...
func TestInboxFullFlagShowsTheWholeBody(t *testing.T) {
	...
	long := strings.Repeat("x", defaultBodyDisplayMax+500)
	newFakeDaemon(t, okHandler(protocol.InboxResult{Messages: []protocol.MessageView{{
		ID: "01K", From: "codex@storefront", Kind: kindNote, Body: long, CreatedAt: ago(time.Second),
	}}}))
...
func TestInboxShortBodyIsNeverTruncated(t *testing.T) {
	setIdentity(t, "frontend", "storefront")
	newFakeDaemon(t, okHandler(protocol.InboxResult{Messages: []protocol.MessageView{{
		ID: "01K", From: "codex@storefront", Kind: kindNote, Body: "short and sweet", CreatedAt: ago(time.Second),
	}}}))
...
// TestAckCommandIsGone confirms `tether ack` is no longer a recognised
...
func TestAckCommandIsGone(t *testing.T) {
	...
	if r.err == nil {
		t.Fatal("`tether ack` was accepted; it should no longer exist")
	}
