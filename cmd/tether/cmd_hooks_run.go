...
"tether hooks install" wires this into settings.json as an async hook.

Blocks on mail via the same daemon call as "tether wait", then either exits
...
"tether hooks install" wires this into settings.json.
...
// drain "tether inbox" performs) and renders it as plain text for an
...
// way the human-readable "tether inbox" view is: a message body is another
...
func renderMailBody(messages []protocol.MessageView, pendingAfter int) string {
	...
	if pendingAfter > 0 {
		fmt.Fprintf(&b, "%d more pending — run `tether inbox`.\n", pendingAfter)
	}

	out := strings.TrimRight(b.String(), "\n")
	if r := []rune(out); len(r) > mailBodyLimit {
		out = string(r[:mailBodyLimit]) + "\n... (truncated, run `tether inbox` for the rest)"
	}
	return out
}
