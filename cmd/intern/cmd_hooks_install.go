...
const hooksInstallLong = `Write intern's Stop and SessionStart hooks into Claude Code's settings.json.
...
command explicitly -- nothing else in intern ever writes to it.`

const hooksStatusLong = `Report whether intern's Claude Code hooks are installed and current.
...
func newHooksInstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  intern hooks install\n" +
			"  intern hooks install --user\n" +
			"  intern hooks install --json",
		Args: cobra.NoArgs,
...
func newHooksStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  intern hooks status\n" +
			"  intern hooks status --user --json",
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
func runHooksInstall(cmd *cobra.Command, opts *hooksTargetOptions) error {
	...
	if res.Changed {
		...
		_, err = fmt.Fprintf(out, "%s already has intern's hooks up to date\n", res.Path)
	}
	return err
}
...
func runHooksStatus(cmd *cobra.Command, opts *hooksTargetOptions) error {
	...
	if !st.StopCurrent || !st.SessionStartCurrent {
		return next(out, "intern hooks install")
	}
	return nil
}
...
// internExecutable resolves the absolute path to this running binary, which
// is what the installed hook command must invoke.
func internExecutable() (string, error) {
