...
const topLong = `Watch the fleet view, refreshing like ` + "`top`" + ` or ` + "`watch intern ls`" + `.

Renders exactly what ` + "`intern ls`" + ` renders, on a timer, until you press Ctrl-C.
Presence is screenshottable this way; messages are not shown here -- read
those with ` + "`intern inbox`" + `.`
...
func renderTopFrame(out io.Writer, workspace string, interval time.Duration) error {
	...
	if _, err := fmt.Fprintf(out, "intern top -- every %s, Ctrl-C to quit -- %s\n\n",
