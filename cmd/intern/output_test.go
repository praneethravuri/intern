...
func TestNextPrintsTheSuggestion(t *testing.T) {
	var buf bytes.Buffer
	if err := next(&buf, `intern send backend "..."`); err != nil {
		t.Fatalf("next: %v", err)
	}
	if got, want := buf.String(), "Next: intern send backend \"...\"\n"; got != want {
...
func TestEmptyPrintsCountAndNext(t *testing.T) {
	var buf bytes.Buffer
	if err := empty(&buf, "agents", "intern register --as <name>"); err != nil {
		t.Fatalf("empty: %v", err)
	}
	want := "0 agents.\nNext: intern register --as <name>\n"
...
func TestNextWriteFailure(t *testing.T) {
	if err := next(failingWriter{}, "intern ls"); err == nil {
		t.Fatal("next with a failing writer: want error, got nil")
	}
}

func TestEmptyWriteFailure(t *testing.T) {
	if err := empty(failingWriter{}, "agents", "intern register --as x"); err == nil {
