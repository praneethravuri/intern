...
func statusResult() protocol.ExplainResult {
	return protocol.ExplainResult{
		Agent: protocol.AgentView{
			...
			StateSource:  "wait",
			StateAgeMS:   4000,
			StateDetail:  "parked in intern wait",
...
func TestExplainShowsStateSourceSeenAndDetail(t *testing.T) {
	...
	requireContains(t, out, "agent", "stdout")
	...
	requireContains(t, out, "parked in intern wait", "stdout")
...
func TestExplainOfAnotherAgentSuggestsSendingToThem(t *testing.T) {
	...
	r := mustRun(t, newExplainCmd(), "", "backend")
	requireContains(t, r.stdout, "Next: intern send backend@storefront", "stdout")
}
...
// applies to the self path -- it must not start demanding --as/$INTERN_NAME
