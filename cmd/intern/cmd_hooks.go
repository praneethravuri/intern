...
"intern hooks install" writes a Stop and a SessionStart hook into Claude
...
Claude Code only, for now: every other harness keeps using ` + "`intern wait`" + `
...
func hooksStateDir() (string, error) {
	...
	return filepath.Join(home, ".intern", "hooks"), nil
}
