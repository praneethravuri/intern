...
func TestDemoEnvIsolatesFromRealHome(t *testing.T) {
	dir := "/tmp/intern-demo-test-xyz"
	env := demoEnv(dir)

	want := []string{
		"HOME=" + dir,
		"INTERN_SOCK=" + dir + "/sock",
		"INTERN_DB=" + dir + "/intern.db",
	}
	for _, w := range want {
		found := false
		for _, e := range env {
...
func TestDialableWithinTimesOutOnADeadSocket(t *testing.T) {
	start := time.Now()
	ok := dialableWithin("/tmp/intern-demo-test-no-such-socket", 60*time.Millisecond)
...
func TestRunDemoStepsReturnsContextErrorWhenAlreadyCancelled(t *testing.T) {
	...
	err := runDemoSteps(ctx, io.Discard, "/nonexistent-intern-binary-for-test", nil)
...
func TestDemoLongDoesNotMentionAnySpecificHarness(t *testing.T) {
	...
	// be described as a real harness.
	for _, bad := range []string{"Claude Code", "Gemini", "Copilot", "OpenCode"} {
