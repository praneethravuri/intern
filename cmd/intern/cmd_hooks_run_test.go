...
// directory, never the real ~/.intern) and $CLAUDE_CODE_SESSION_ID (so the
// single-flight/budget key is deterministic) -- set directly rather than via
// $INTERN_SESSION_ID because detectHarness checks CLAUDE_CODE_SESSION_ID
...
func TestHooksRunStopDeliversMailAsABlockingExit(t *testing.T) {
	...
	if !ok {
		t.Fatalf("stderr is not a intern envelope: %q", r.stderr)
	}
...
func TestHooksRunSessionStartInjectsPendingMail(t *testing.T) {
	...
	if !ok {
		t.Fatalf("additionalContext is not a intern envelope: %q", out.HookSpecificOutput.AdditionalContext)
	}
