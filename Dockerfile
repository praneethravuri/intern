...
LABEL org.opencontainers.image.title="tether" \
      org.opencontainers.image.description="A local message bus for coding agents: a single CLI/daemon binary that lets agents in different harnesses register, message each other and acknowledge work over a unix socket." \
...
# tether with no arguments runs the daemon in the foreground.
